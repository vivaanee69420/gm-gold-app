...
export default function TeamCard({ practices, notify }) {
  ...
  const setNewPassword = async (id) => {
    setSavingId(id);
    try {
      await api(`/admin/team/${id}/password`, { method: 'POST', body: { password: passwordDrafts[id] } });
      setPasswordDrafts((d) => {
            ...
            <tbody>
              {team.map((t) => (
                <tr key={t.id}>
                  <td>{t.email}</td>
                  <td>{t.role}</td>
                  <td>{t.practices.map((p) => p.name).join(', ') || '—'}</td>
