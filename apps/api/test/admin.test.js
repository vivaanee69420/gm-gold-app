...
beforeAll(async () => {
  ...
  await request(app).put('/admin/reward-amount').set(auth(agents.referrer)).send({ amountPennies: 2000 });
});
...
describe('client events and funnel (FR-28)', () => {
  ...
  it('records client events and reports the tripwire completion rate', async () => {
    ...
    const res = await request(app).get('/admin/reports/funnel').set(auth(agents.referrer));
    expect(res.status).toBe(200);
  ...
  it('carries the hand-entered invite count (marked manual)', async () => {
    const put = await request(app).put('/admin/settings').set(auth(agents.referrer))
...
describe('referral review queue (FR-25)', () => {
  it('lists suspects; clearing keeps the pipeline creditable', async () => {
    ...
    const list = await request(app).get('/admin/referral-review').set(auth(agents.referrer));
    ...
    const after = await request(app).get('/admin/referral-review').set(auth(agents.referrer));
...
describe('top referrers (FR-25)', () => {
  it('ranks referrers by credited amount', async () => {
    const res = await request(app).get('/admin/reports/top-referrers').set(auth(agents.referrer));
    expect(res.status).toBe(200);
...
describe('admin payout cancel (FR-21)', () => {
  it('requires a reason, cancels, notifies the member, and keeps the balance', async () => {
    ...
    const noReason = await request(app)
      .post(`/admin/payouts/${payoutId}/cancel`).set(auth(agents.referrer)).send({});
    expect(noReason.status).toBe(422);
    expect(noReason.body.error).toBe('reason_required');

    const cancelled = await request(app)
      .post(`/admin/payouts/${payoutId}/cancel`).set(auth(agents.referrer))
      .send({ reason: 'member could not attend' });
    expect(cancelled.status).toBe(200);

    const list = await request(app).get('/admin/payouts').set(auth(agents.referrer));
...
describe('session revocation (FR-03)', () => {
  it('kills tokens issued before the revocation', async () => {
    ...
    const revoke = await request(app)
      .post(`/admin/users/${user.id}/revoke-sessions`).set(auth(agents.referrer));
...
describe('admin_users practice scoping (FR-24)', () => {
  it('a practice-scoped admin sees only their practices; owners see all', async () => {
    if (!agents.otherPracticeId) return; // needs two seeded practices
    const scoped = await signIn('07700 900806');
    await db.query(
      `insert into admin_users (user_id, email, role, practice_ids)
       values ($1, 'frontdesk@gmdental.co.uk', 'admin', array[$2::uuid])`,
      [scoped.user.id, agents.otherPracticeId],
    );

    const payouts = await request(app).get('/admin/payouts').set(auth(scoped.token));
    expect(payouts.status).toBe(200);
    expect(payouts.body.payouts).toEqual([]); // Sarah's payout sits at the first practice

    const referrals = await request(app).get('/admin/referrals').set(auth(scoped.token));
    expect(referrals.body.referrals).toEqual([]);

    const all = await request(app).get('/admin/payouts').set(auth(agents.referrer));
    expect(all.body.payouts.length).toBeGreaterThan(0); // dev fallback acts as owner
  });
});
